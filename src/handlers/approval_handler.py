                        ...
                        try:
                            ...
                            self.mattermost_client.send_dm(
                                ...
                                message=f"✅ AWS Role이 생성되었습니다!\n\n"
                                       ...
                                       f"---\n"
                                       f"## 💻 CLI에서 사용하기\n\n"
                                       f"> ⚠️ `--duration-seconds {role_info.get('max_session_duration', 3600)}` 옵션이 반드시 포함되어야 요청 기간 동안 끊기지 않습니다. 빠지면 default 1시간 후 ExpiredToken 에러 발생.\n\n"
                                       ...
                                       f"CREDS=$(aws sts assume-role --role-arn {role_info['role_arn']} --role-session-name {request.iam_user_name}-session --duration-seconds {role_info.get('max_session_duration', 3600)} --query 'Credentials' --output json)\n\n"
                                       f"# 2. 환경변수 설정\n"
                                       ...
                                       f"# 1. assume-role 실행 (--duration-seconds 명시)\n"
                                       f'$creds = (aws sts assume-role --role-arn {role_info["role_arn"]} --role-session-name {request.iam_user_name}-session --duration-seconds {role_info.get("max_session_duration", 3600)} --query "Credentials" --output json) | ConvertFrom-Json\n\n'
                                       f"# 2. 환경변수 설정\n"
